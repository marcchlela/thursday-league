...
export function NotificationOnboarding({ userId }: { userId: string }) {
  ...
  const refreshDevice = useCallback(async () => {
    ...
    setDevice(next);
    if (next.enabled) {
      if (readNotificationOnboardingStatus(userId) !== "completed") {
        saveNotificationOnboardingStatus(userId, "completed");
      }
      setOpen(false);
    }
    return next;
  ...
  useEffect(() => {
    ...
    async function initialize() {
      ...
      const hasInstallHandoff = next.installed
        ? await consumeNotificationInstallHandoff(userId)
        : false;
      if (cancelled) return;
      ...
      if (explicitlyRequested || hasInstallHandoff) {
        timeout = window.setTimeout(() => {
          setStep(hasInstallHandoff ? 1 : requestedStep === 1 ? 1 : 0);
          setMessage(null);
          setOpen(true);
        }, explicitlyRequested ? 0 : 350);
      }
    }

    function openOnboarding(event: Event) {
      const requestedStep = (event as CustomEvent<{ step?: number }>).detail?.step;
      setStep(requestedStep === 1 ? 1 : 0);
      setMessage(null);
      setOpen(true);
      void refreshDevice();
    }
  ...
  }, [refreshDevice, userId]);

  useEffect(() => {
    if (!open || step !== 0 || !device.isIOS || device.installed) return;
    void markNotificationInstallHandoff(userId).catch(() => {
      setMessage("We could not prepare the installed-app handoff. You can still continue setup after signing in there.");
    });
  }, [device.installed, device.isIOS, open, step, userId]);
  ...
  function onTouchEnd(event: React.TouchEvent) {
    ...
    if (deltaX < 0 && step < 1) next();
    if (deltaX > 0 && step > 0) previous();
  }

  return (
    <Modal open={open} title="Device notification setup" onClose={dismiss}>
      <div
      ...
      >
        <div className="flex items-center justify-between gap-3">
          <Pill>Device setup</Pill>
          <div className="grid w-24 grid-cols-2 gap-1.5" aria-label={`Step ${step + 1} of 2`}>
            {[0, 1].map(index => <span key={index} className={`h-1.5 rounded-full transition-colors ${index <= step ? "bg-league-gold" : "bg-league-gold/15"}`} />)}
          </div>
        </div>

        {step === 0 ? (
          <div className="mt-8">
            <div className="grid h-16 w-16 place-items-center rounded-3xl border border-league-gold/25 bg-perimeter-400/[.08] text-perimeter-400"><Smartphone size={30} /></div>
            <h2 className="mt-5 font-display text-4xl uppercase">Add it to your Home Screen</h2>
            {device.installed ? (
              <div className="mt-5 flex items-center gap-3 rounded-2xl border border-league-gold/25 bg-perimeter-400/[.08] p-4 text-perimeter-400"><CheckCircle2 /> Thursday League is already running as an installed app.</div>
            ) : device.isIOS ? (
              <div className="mt-5 space-y-3 text-sm text-chalk/70">
                <div className="flex gap-3 rounded-2xl border border-league-gold/15 bg-league-gold/[.025] p-3"><Share className="shrink-0 text-perimeter-400" size={20} /><span>Press the browser <strong>Share</strong> button.</span></div>
                <div className="flex gap-3 rounded-2xl border border-league-gold/15 bg-league-gold/[.025] p-3"><Download className="shrink-0 text-perimeter-400" size={20} /><span>Choose <strong>Add to Home Screen</strong>, then Add.</span></div>
                <div className="flex gap-3 rounded-2xl border border-league-gold/15 bg-league-gold/[.025] p-3"><AppWindow className="shrink-0 text-perimeter-400" size={20} /><span>Open Thursday League and finish setup.</span></div>
        ...
        {step === 1 ? (
          <div className="mt-8">
            <div className="grid h-16 w-16 place-items-center rounded-3xl border border-league-gold/25 bg-perimeter-400/[.08] text-perimeter-400"><ShieldCheck size={30} /></div>
            ...
            {device.isIOS && !device.installed ? (
              ...
              <div className="mt-5 flex items-center gap-3 rounded-2xl border border-league-gold/25 bg-perimeter-400/[.08] p-4 text-perimeter-400"><CheckCircle2 /> Notifications are enabled on this device.</div>
            ) : (
        ...
        <div className="mt-7 flex items-center justify-between gap-2 border-t border-league-gold/18 pt-4">
          ...
          <div className="flex gap-2">
            ...
            {step < 1 ? <PrimaryButton type="button" onClick={next} className="inline-flex items-center gap-2">Next <ChevronRight size={18} /></PrimaryButton> : null}
          </div>
        </div>
      </div>
    </Modal>
...
export function NotificationNudge({ userId }: { userId: string }) {
  ...
  const refresh = useCallback(async () => {
    ...
    setVisible(!device.enabled && status !== "completed");
    setBlocked(device.permission === "denied");
    setSetupStep(device.permission === "denied" || device.installed || (device.supported && !device.isIOS) ? 1 : 0);
  }, [userId]);
