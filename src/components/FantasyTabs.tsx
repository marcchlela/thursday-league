  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex rounded-3xl border border-white/10 bg-white/[0.03] p-1">
        {[["set", "Set Team"], ["standings", "Standings"], ["history", "History"]].map(([key, label]) => <button key={key} onClick={() => setTab(key as "set" | "standings" | "history")} className={`flex-1 rounded-2xl px-4 py-3 font-bold transition ${tab === key ? "bg-perimeter-400/20 text-chalk ring-1 ring-perimeter-400/30" : "text-chalk/55 hover:text-chalk"}`}>{label}</button>)}
      </div>
...
function activeFantasyGame(data: LeagueData) {
  return [...data.games].filter(g => g.status === "draft" || g.status === "live").sort((a, b) => new Date(a.game_date).getTime() - new Date(b.game_date).getTime())[0];
}
...
function SetTeam({ data, reload }: { data: LeagueData; reload: () => void }) {
  ...
  const lockState = game.status === "final" ? { label: "Final", text: "This game is final. Picks are locked." } : game.status === "live" ? { label: "Live - locked", text: "The game is live. Picks can no longer be changed." } : { label: "Lineup set - open", text: `Editable until kickoff: ${formatDateTime(game.game_date)}` };

  async function savePicks(draft: Omit<FantasyPick, "id" | "squad_id" | "created_at">[]) {
    let squadId = squad?.id;
    if (!squadId) {
      const { data: newSquad, error } = await supabase.from("fantasy_squads").insert({ user_id: user!.id, game_id: game!.id }).select("*").single();
      if (error) throw error;
      squadId = newSquad.id;
    } else {
      await supabase.from("fantasy_picks").delete().eq("squad_id", squadId);
    }
    const { error } = await supabase.from("fantasy_picks").insert(draft.map(p => ({ ...p, squad_id: squadId })));
    if (error) throw error;
    reload();
  }

  return <div className="space-y-4"><Card><div className="flex flex-wrap items-center justify-between gap-3"><div><Pill className={locked ? "border-floodlight/40 bg-floodlight/20 text-floodlight" : "border-perimeter-400/40 bg-perimeter-400/10 text-perimeter-400"}>{lockState.label}</Pill><h2 className="mt-2 font-display text-3xl uppercase">Locks at kickoff</h2><p className="text-chalk/60">{lockState.text}</p></div><div className="font-mono text-2xl text-chalk/70">{initialPicks.length ? "Your picks are in" : "No picks yet"}</div></div></Card>{lineups.length ? <PitchPicker players={data.players} lineups={lineups} extraPlayers={extraPlayers} initialPicks={initialPicks} locked={locked} onSave={savePicks} /> : <EmptyState title="Lineup pending" text="This game exists, but the admin has not picked who's playing yet." />}</div>;
}
...
function Standings({ data }: { data: LeagueData }) {
  ...
  return <Card className="mx-auto max-w-3xl"><div className="flex items-center justify-between gap-3"><div><h2 className="font-display text-3xl uppercase">Points standings</h2><p className="text-sm text-chalk/55">All-time fantasy points. No season reset.</p></div>{currentGame?.status === "draft" ? <Pill className="border-floodlight/40 bg-floodlight/10 text-floodlight">{notPicked} haven't picked yet</Pill> : null}</div><div className="mt-5 space-y-2">{board.map(row => <div key={row.userId} className="grid grid-cols-[auto_1fr_auto] items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.03] p-3"><span className="grid h-9 w-9 place-items-center rounded-xl bg-perimeter-400/15 font-mono text-perimeter-400">#{row.rank}</span><span className="font-semibold">{row.username}</span><span className="font-mono text-xl">{row.points}</span></div>)}</div></Card>;
}
...
function History({ data }: { data: LeagueData }) {
  ...
  return <div className="grid gap-6 lg:grid-cols-[.85fr_1.15fr]"><Card><h2 className="font-display text-3xl uppercase">Past games</h2><div className="mt-4 space-y-2">{finals.map(g => <button key={g.id} onClick={() => setSelectedGameId(g.id)} className={`w-full rounded-2xl border p-3 text-left transition ${game?.id === g.id ? "border-perimeter-400/60 bg-perimeter-400/10" : "border-white/10 bg-white/[0.03] hover:bg-white/[0.06]"}`}><div className="font-semibold">{formatDateTime(g.game_date)}</div><div className="text-sm text-chalk/50">POTM: {playerName(data.players, g.potm_player_id)}</div></button>)}</div></Card><Card><h2 className="font-display text-3xl uppercase">Weekly leaderboard</h2><p className="mt-1 text-sm text-chalk/50">Open any manager to see their squad.</p><div className="mt-4 space-y-2">{ordered.map(row => <Link href={`/fantasy/history/${game.id}/${row.userId}`} key={row.userId} className={`grid w-full grid-cols-[auto_1fr_auto] items-center gap-3 rounded-2xl border p-3 text-left transition hover:border-perimeter-400/50 hover:bg-perimeter-400/10 ${row.userId === user?.id ? "border-floodlight/50 bg-floodlight/10" : "border-white/10 bg-white/[0.03]"}`}><span className="font-mono text-perimeter-400">#{row.rank}</span><span>{row.username}{row.userId === user?.id ? " · you" : ""}</span><span className="font-mono text-xl">{row.points}</span></Link>)}</div></Card></div>;
}
