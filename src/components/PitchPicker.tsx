...
export function PitchPicker({
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      <Modal open={selectedSlot != null && !selectingCaptain} title="Choose a player" onClose={() => setSelectedSlot(null)}>
        ...
        <div className="mt-5 max-h-[55vh] space-y-2 overflow-y-auto pr-1">
          {candidates.map(item => {
            ...
            return <button key={item.player!.id} type="button" onClick={() => choosePlayer(item.player!.id)} className="flex w-full items-center justify-between gap-3 rounded-xl border border-white/[.07] bg-white/[.025] px-3 py-3 text-left transition hover:border-league-gold/35 hover:bg-league-gold/[.055] focus:outline-none focus-visible:ring-2 focus-visible:ring-league-gold"><span className="min-w-0 truncate font-semibold">{item.player!.name}</span><span className="shrink-0 text-[10px] font-bold uppercase tracking-wider text-chalk/35">{alreadyPicked ? "Swap" : item.team ? `Team ${item.team}` : "Available"}</span></button>;
          })}
          {!candidates.length ? <p className="rounded-xl border border-dashed border-white/10 p-6 text-center text-sm text-chalk/40">No eligible players are available for this slot.</p> : null}
        </div>
      </Modal>
          ...
          return (
            ...
            >
              <span className="relative mx-auto block h-[4.9rem] w-[4.9rem] sm:h-[5.8rem] sm:w-[5.8rem]">
                ...
                {!player ? <span className="absolute inset-0 grid place-items-center"><span className="grid h-7 w-7 place-items-center rounded-full border border-white/25 bg-ink-900/80 text-chalk"><Plus size={15} /></span></span> : null}
      ...
      {locked ? <div className="mt-3 rounded-xl border border-white/[.07] bg-[#171814] px-4 py-3 text-center text-sm text-chalk/45">Picks are locked for this match.</div> : null}
