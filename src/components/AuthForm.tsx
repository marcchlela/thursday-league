      ...
      <div className="relative mx-auto grid min-h-screen max-w-6xl items-center gap-7 px-4 pb-[calc(2rem+env(safe-area-inset-bottom))] pt-[calc(2rem+env(safe-area-inset-top))] sm:px-6 lg:grid-cols-[1.05fr_.95fr] lg:gap-16 lg:py-12">
        ...
        <form onSubmit={submit} className="rounded-[1.45rem] border border-league-gold/30 bg-ink-850 p-4 shadow-[0_16px_44px_rgba(0,0,0,.28)] sm:p-6 lg:p-7">
          ...
          <div className="space-y-4">
            ...
            {mode === "signup" ? <div className="block"><label htmlFor="auth-email" className="mb-2 flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-chalk/45"><Mail size={13} />Email</label><TextInput id="auth-email" type="email" value={email} onChange={event => setEmail(event.target.value)} placeholder="you@example.com" autoComplete="email" autoCapitalize="none" aria-describedby="auth-email-help" className="rounded-xl border-league-gold/15 py-3" /><span id="auth-email-help" className="mt-1.5 block text-[11px] leading-relaxed text-chalk/32">Used privately for verification and password recovery.</span></div> : null}
            <div className="block">
