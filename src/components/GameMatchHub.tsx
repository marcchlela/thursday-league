  ...
  return (
    <section className="relative min-h-[15.5rem] overflow-hidden rounded-[1.6rem] border border-league-gold/30 bg-[#151813] p-4 shadow-[0_14px_35px_rgba(0,0,0,.22),inset_0_1px_0_rgba(218,165,32,.07)] md:min-h-[17rem] md:p-6">
      ...
      <div className="relative flex items-center justify-between">
        ...
        <span className={cn("rounded-full px-2.5 py-1 text-[10px] font-black uppercase tracking-widest", game.status === "live" ? "bg-red-400/15 text-red-300" : "bg-white/[.05] text-chalk/40")}>{game.status}</span>
      </div>
      ...
      {game.status === "final" && game.potm_player_id ? (
        ...
        <div className={cn("relative mx-auto mt-3 flex w-fit items-center gap-2 rounded-full border px-3 py-1.5 text-xs font-bold", ready ? "border-turf-400/20 bg-turf-400/10 text-turf-100" : "border-white/[.09] bg-black/20 text-chalk/55")}>
...
function MatchTabs({ active, onChange }: { active: MatchDetailTab; onChange: (tab: MatchDetailTab) => void }) {
  ...
  return (
    <div className="grid grid-cols-4 overflow-hidden rounded-[1.15rem] border border-league-gold/25 bg-[#171814] p-1 shadow-[0_7px_20px_rgba(0,0,0,.13)]" role="tablist" aria-label="Match details">
...
function MatchPitch({ game, data, lineups }: { game: Game; data: LeagueData; lineups: GameLineup[] }) {
  return (
    <section className="relative mx-auto aspect-[3/5] min-h-[620px] w-full max-w-2xl overflow-hidden rounded-[1.7rem] border-2 border-league-gold/40 bg-[#0b3e22] shadow-[0_16px_38px_rgba(0,0,0,.25),inset_0_0_45px_rgba(0,0,0,.24)]" aria-label="Full match lineup pitch">
      ...
      <div className="absolute left-1/2 top-3 h-7 w-20 -translate-x-1/2 border-x border-b border-chalk/35" />
      <div className="absolute bottom-3 left-1/2 h-7 w-20 -translate-x-1/2 border-x border-t border-chalk/35" />
  ...
  const content = (
    <>
      <div className="relative mx-auto h-12 w-12 sm:h-14 sm:w-14">
        ...
        <span className={cn("absolute -right-2 top-0 rounded-md border bg-ink-900/90 px-1 py-0.5 text-[7px] font-black uppercase tracking-wider shadow-sm", team === "A" ? "border-turf-400/25 text-turf-100" : "border-red-400/25 text-red-200")}>{role === "goalkeeper" ? "GK" : "OUT"}</span>
      </div>
      <div className={cn("mx-auto -mt-0.5 truncate rounded-lg border bg-ink-900/90 px-1.5 py-1 text-[9px] font-bold shadow-lg backdrop-blur sm:text-[11px]", team === "A" ? "border-turf-400/25" : "border-red-400/25")}>{name}</div>
    </>
  );
...
function MatchFantasy({ game, data }: { game: Game; data: LeagueData }) {
  if (game.status !== "final") {
    return (
      <section className="rounded-[1.35rem] border border-league-gold/25 bg-[#171814] p-6 text-center shadow-[0_9px_24px_rgba(0,0,0,.13)] md:p-8">
