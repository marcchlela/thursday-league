...
function GamesViewTabs({ active, onChange }: { active: GamesView; onChange: (view: GamesView) => void }) {
  ...
  return (
    <div className="grid grid-cols-2 rounded-[1.15rem] border border-league-gold/25 bg-[#171814] p-1 shadow-[0_7px_20px_rgba(0,0,0,.13)]" role="tablist" aria-label="Games views">
...
function GameSelector({ games, selectedId, onChange }: { games: Game[]; selectedId: string; onChange: (gameId: string) => void }) {
  return <label className="block rounded-[1.1rem] border border-league-gold/20 bg-[#171814] p-2.5 sm:flex sm:items-center sm:gap-3"><span className="mb-2 block shrink-0 px-1 text-[10px] font-black uppercase tracking-widest text-chalk/35 sm:mb-0">Choose fixture</span><Select value={selectedId} onChange={event => onChange(event.target.value)} className="rounded-xl border-white/[.07] py-2 text-sm">{games.map(game => <option key={game.id} value={game.id}>{formatDateTime(game.game_date)}{game.status === "live" ? " · Live" : ""}</option>)}</Select></label>;
}
  ...
  return (
    <section aria-label="Match results" className="space-y-3">
      {games.map(game => {
        ...
        return (
          <Link key={game.id} href={`/games/${game.id}`} className="group relative grid min-h-[6.5rem] grid-cols-[1fr_auto_1fr] items-center gap-2 rounded-[1.3rem] border border-league-gold/25 bg-[#171814] px-3 pb-3 pt-7 shadow-[0_9px_24px_rgba(0,0,0,.13)] transition hover:border-league-gold/45 focus:outline-none focus-visible:ring-2 focus-visible:ring-league-gold sm:gap-5 sm:px-5">
...
function GamesPageSkeleton({ view }: { view: GamesView }) {
  return (
    <div className="mx-auto max-w-5xl space-y-4 md:space-y-5" role="status" aria-label="Loading games">
      <div className="grid grid-cols-2 rounded-[1.15rem] border border-league-gold/25 bg-[#171814] p-1"><div className={cn("rounded-[.85rem] px-4 py-3 text-center text-sm font-extrabold", view === "upcoming" ? "bg-turf-400/[.07] text-turf-400/55" : "text-chalk/25")}>Upcoming</div><div className={cn("rounded-[.85rem] px-4 py-3 text-center text-sm font-extrabold", view === "results" ? "bg-turf-400/[.07] text-turf-400/55" : "text-chalk/25")}>Results</div></div>
      {view === "upcoming" ? <><section className="min-h-[15.5rem] rounded-[1.6rem] border border-league-gold/30 bg-[#151813] p-4 md:min-h-[17rem] md:p-6"><div className="flex items-center justify-between"><span className="text-xs font-black uppercase tracking-[.18em] text-turf-400">Next match</span><span className="text-[10px] font-bold uppercase tracking-widest text-chalk/25">Loading fixture</span></div><div className="mt-6 grid grid-cols-[1fr_auto_1fr] items-center gap-3"><SkeletonTeam /><span className="font-display text-3xl uppercase text-chalk/15">vs</span><SkeletonTeam /></div><div className="skeleton-shimmer mx-auto mt-5 h-4 w-40 rounded-full" /><div className="skeleton-shimmer mx-auto mt-4 h-7 w-36 rounded-full" /></section><div className="grid grid-cols-4 rounded-[1.15rem] border border-league-gold/25 bg-[#171814] p-1">{["Lineups", "Stats", "Fantasy", "Bets"].map((label, index) => <div key={label} className={cn("rounded-[.85rem] px-1 py-2.5 text-center text-[11px] font-extrabold sm:text-sm", index === 0 ? "bg-league-gold/[.07] text-league-gold/50" : "text-chalk/20")}>{label}</div>)}</div><div className="relative mx-auto aspect-[3/5] min-h-[620px] w-full max-w-2xl overflow-hidden rounded-[1.7rem] border-2 border-league-gold/30 bg-[#0b3e22]/75"><div className="absolute inset-3 rounded-[1.25rem] border border-chalk/15" /><div className="absolute inset-x-3 top-1/2 border-t border-chalk/15" />{Array.from({ length: 10 }, (_, index) => <div key={index} className="skeleton-shimmer absolute h-10 w-20 rounded-xl" style={{ left: `${index % 2 ? 65 : 20}%`, top: `${8 + Math.floor(index / 2) * 20}%` }} />)}</div></> : <div className="space-y-3">{Array.from({ length: 5 }, (_, index) => <div key={index} className="skeleton-shimmer h-[6.5rem] rounded-[1.3rem] border border-league-gold/20" />)}</div>}
