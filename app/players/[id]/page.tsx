...
export default function PlayerDetailPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-4 md:space-y-5">
      ...
      <section className="relative overflow-hidden rounded-[1.45rem] border border-league-gold/30 bg-[#171814] p-4 shadow-[0_10px_28px_rgba(0,0,0,.16)] sm:p-5">
      ...
      <section className="flex items-center justify-between gap-4 rounded-[1.2rem] border border-league-gold/25 bg-[#171814] p-4 shadow-[0_7px_20px_rgba(0,0,0,.1)] sm:px-5">
        <div className="min-w-0">
                  ...
                  className={cn(
                    ...
                    performance.result === "W"
                      ...
                      : performance.result === "L"
                        ...
                        : "border-white/15 bg-white/[.07] text-chalk/60"
                  )}
                >
                  {performance.result}
                </Link>
...
function PlayerStat({ value, label, icon: Icon, muted = false, warning = false }: { value: number | string; label: string; icon: React.ComponentType<{ size?: number; className?: string }>; muted?: boolean; warning?: boolean }) {
  return (
    <div className={cn("bg-[#171814] px-2 py-4 text-center sm:p-5", muted && "opacity-35")}>
...
function SummaryCard({ icon: Icon, value, label, gold = false }: { icon: React.ComponentType<{ size?: number; className?: string }>; value: number | string; label: string; gold?: boolean }) {
  return (
    <section className="min-w-0 rounded-[1.2rem] border border-league-gold/25 bg-[#171814] p-3 text-center shadow-[0_7px_20px_rgba(0,0,0,.12)] sm:p-4">
...
function RateStat({ value, label }: { value: string; label: string }) {
  return <div className="bg-[#171814] p-4 text-center sm:p-5"><div className="font-mono text-xl font-black text-turf-400 sm:text-2xl">{value}</div><div className="mt-1 text-[7px] font-black uppercase tracking-[.12em] text-chalk/28 sm:text-[9px]">{label}</div></div>;
}
...
function PlayerDetailSkeleton() {
  return (
    <div className="mx-auto max-w-4xl space-y-4" role="status" aria-label="Loading player profile">
      ...
      <div className="flex min-h-36 items-center gap-4 rounded-[1.45rem] border border-league-gold/20 bg-[#171814] p-4"><div className="skeleton-shimmer h-24 w-24 rounded-full" /><div className="flex-1"><div className="skeleton-shimmer h-3 w-24 rounded" /><div className="skeleton-shimmer mt-3 h-10 w-56 max-w-full rounded-lg" /><div className="skeleton-shimmer mt-3 h-6 w-32 rounded-full" /></div></div>
      <div className="skeleton-shimmer h-20 rounded-[1.2rem] border border-league-gold/15" />
      <div className="flex h-[4.5rem] items-center justify-between rounded-[1.2rem] border border-league-gold/15 bg-[#171814] px-4"><div className="skeleton-shimmer h-8 w-24 rounded-lg" /><div className="flex gap-2">{Array.from({ length: 5 }, (_, index) => <div key={index} className="skeleton-shimmer h-8 w-8 rounded-full" />)}</div></div>
