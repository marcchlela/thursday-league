...
function NextMatch({ game, lineupsReady }: { game: Game; lineupsReady: boolean }) {
  ...
  return (
    <Link href={`/games/${game.id}`} className="group relative block min-h-[15.5rem] overflow-hidden rounded-[1.6rem] border border-league-gold/30 bg-ink-850 p-4 shadow-[0_14px_35px_rgba(0,0,0,.22),inset_0_1px_0_rgba(218,165,32,.07)] transition hover:border-league-gold/50 focus:outline-none focus-visible:ring-2 focus-visible:ring-league-gold md:min-h-[17rem] md:p-6">
      ...
      <div className={cn("relative mx-auto mt-3 flex w-fit items-center gap-2 rounded-full border px-3 py-1.5 text-xs font-bold", lineupsReady ? "border-turf-400/20 bg-turf-400/10 text-turf-100" : "border-chalk/[.09] bg-black/20 text-chalk/55")}>
...
function RecentMatch({ game, data }: { game?: Game; data: LeagueData }) {
  ...
  return (
    <section>
      ...
      {game && score ? (
        <Link href={`/games/${game.id}`} className="group grid min-h-[7rem] grid-cols-[1fr_auto_1fr] items-center gap-3 rounded-[1.35rem] border border-league-gold/25 bg-ink-850 p-3.5 shadow-[0_9px_24px_rgba(0,0,0,.14),inset_0_1px_0_rgba(218,165,32,.05)] transition hover:border-league-gold/45 focus:outline-none focus-visible:ring-2 focus-visible:ring-league-gold md:min-h-[8rem] md:p-5">
        ...
        <div className="rounded-[1.35rem] border border-dashed border-league-gold/25 bg-chalk/[.025] px-4 py-8 text-center text-sm text-chalk/45">The first result will appear here.</div>
      )}
    </section>
  );
}
...
function LeaderTable({ title, icon: Icon, rows, tone }: { title: string; icon: IconType; rows: { id: string; name: string; value: number }[]; tone: "green" | "gold" }) {
  return (
    <section className="overflow-hidden rounded-[1.35rem] border border-league-gold/25 bg-ink-850 shadow-[0_9px_24px_rgba(0,0,0,.13),inset_0_1px_0_rgba(218,165,32,.05)]">
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-4 md:space-y-5" role="status" aria-live="polite" aria-label="Loading Home">
      ...
      <section>
        ...
        <div className="grid min-h-[7rem] grid-cols-[1fr_auto_1fr] items-center gap-3 rounded-[1.35rem] border border-league-gold/25 bg-ink-850 p-3.5 shadow-[0_9px_24px_rgba(0,0,0,.14),inset_0_1px_0_rgba(218,165,32,.05)] md:min-h-[8rem] md:p-5">
          <SkeletonCompactTeam />
...
function SkeletonBlock({ className }: { className: string }) {
  return <div aria-hidden="true" className={cn("skeleton-shimmer bg-chalk/[.045]", className)} />;
}
...
function SkeletonAction({ title, tone }: { title: string; tone: "green" | "gold" }) {
  return (
    <div className={cn("min-h-[7.25rem] rounded-[1.35rem] border border-league-gold/25 p-3.5 shadow-[0_9px_24px_rgba(0,0,0,.16),inset_0_1px_0_rgba(218,165,32,.06)] md:min-h-[8rem] md:p-5", tone === "green" ? "bg-turf-400/[.07]" : "bg-league-gold/[.07]")}>
...
function NoUpcomingMatch() {
  return (
    <section className="relative min-h-[13rem] overflow-hidden rounded-[1.6rem] border border-dashed border-league-gold/30 bg-ink-850 p-5 shadow-[0_14px_35px_rgba(0,0,0,.18),inset_0_1px_0_rgba(218,165,32,.06)]">
