...
import { Card, EmptyState, ErrorState, LoadingState, Modal, Pill, Stat } from "@/components/ui";
...
export default function FantasySquadHistoryPage() {
  ...
  return <div className="space-y-6">
    <Link href={`/fantasy?tab=history&game=${game.id}`} className="inline-flex items-center gap-2 rounded-2xl border border-white/10 bg-white/5 px-4 py-2 text-sm font-semibold text-chalk/70 transition hover:text-chalk"><ArrowLeft size={16} /> Back to Fantasy history</Link>
    ...
    <Modal open={!!selectedPlayer} title={selectedPlayer?.playerName || "Player points"} onClose={() => setSelectedPlayer(null)}>{selectedPlayer ? <><div className="flex items-start justify-between gap-3"><div><Pill>{selectedPlayer.role === "goalkeeper" ? "Goalkeeper" : "Outfield"}</Pill><h2 className="mt-2 font-display text-4xl uppercase">{selectedPlayer.playerName}</h2></div><button onClick={() => setSelectedPlayer(null)} className="rounded-xl border border-white/10 p-2 text-chalk/60 hover:text-chalk" aria-label="Close player points"><X size={18} /></button></div><div className="mt-5 space-y-2">{selectedPlayer.lines.map((line, index) => <div key={index} className="rounded-2xl border border-white/10 bg-white/[0.03] px-3 py-2 text-sm text-chalk/70">{line}</div>)}</div><div className="mt-4 flex items-center justify-between border-t border-white/10 pt-4"><span className="font-bold uppercase tracking-wider text-chalk/50">Total</span><span className="font-mono text-3xl text-perimeter-400">{selectedPlayer.points}</span></div></> : null}</Modal>
  </div>;
}
